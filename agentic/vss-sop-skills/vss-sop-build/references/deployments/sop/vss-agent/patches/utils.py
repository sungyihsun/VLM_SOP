...
def parse_vst_sensor_list_response(sensors_str) -> set[str]:
    """
    Parse VST sensor list response into a set of sensor names.

    Supports:
    - Pydantic objects with sensor_names attribute (e.g., VSTSensorListOutput)
    - Dict with sensor_names key
    - VSTSensorListOutput JSON string: {"sensor_names": ["name1", "name2", ...]}
    - Legacy format: {"sensor_id": {"name": "...", "sensorId": "...", ...}, ...}

    Args:
        sensors_str: Response from VST sensor list tool (string, dict, or Pydantic object)

    Returns:
        Set of sensor names extracted from the response (always set[str], empty on parse failure).
    """
    if hasattr(sensors_str, "sensor_names"):
        return set(sensors_str.sensor_names)
    if isinstance(sensors_str, dict) and "sensor_names" in sensors_str:
        return set(sensors_str["sensor_names"])

    if not isinstance(sensors_str, str):
        try:
            sensors_str = json.dumps(sensors_str)
        except (TypeError, ValueError):
            sensors_str = str(sensors_str)
