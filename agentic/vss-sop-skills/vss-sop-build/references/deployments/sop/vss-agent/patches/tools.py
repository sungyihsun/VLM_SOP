...
class GetSensorIdsInput(BaseModel):
    ...
    @classmethod
    def normalize_null_place(cls, data):
        if hasattr(data, "get") and data.get("place") in ("None", "", "null", "undefined"):
            data["place"] = None
        return data
...
class GetSOPStatusInput(BaseModel):
    """Input for get_sop_status function."""

    sensor_id: str | None = Field(default=None, description="Optional sensor ID to filter by")
    start_time: str | None = Field(default=None, description="Optional start time (ISO format: YYYY-MM-DDTHH:MM:SS.sssZ)")
    end_time: str | None = Field(default=None, description="Optional end time (ISO format: YYYY-MM-DDTHH:MM:SS.sssZ)")
    max_count: int = Field(default=10, description="Maximum number of messages to return (default: 10)")

    @model_validator(mode="before")
    @classmethod
    def normalize_null_values(cls, data):
        if hasattr(data, "get"):
            for key in ("sensor_id", "start_time", "end_time"):
                if data.get(key) in ("None", "", "null", "undefined"):
                    data[key] = None
        return data
...
class GetSOPReportInput(BaseModel):
    """Input for get_sop_report function."""
...
class GetSOPAsIncidentsInput(BaseModel):
    """Adapter input matching GetIncidentsInputBase schema for report_agent compatibility.

    report_agent internally calls get_incidents_tool with incident-style parameters.
    This adapter accepts those parameters and translates them to SOP tool calls.
    """

    source: str | None = Field(default=None, description="Source sensor ID or place name")
    source_type: str | None = Field(default=None, description="Type of source ('sensor' or 'place')")
    ...
    max_count: int = Field(default=10, description="Maximum number of results")
    includes: list[str] | None = Field(default=None, description="Metadata fields to include (ignored for SOP)")
...
class GetSOPAsIncidentInput(BaseModel):
    """Adapter input matching GetIncidentInput schema for report_agent compatibility.

    report_agent internally calls get_incident_tool with an incident ID.
    This adapter accepts that ID (encoded as sensor_id::start_time::end_time)
    and translates it to a get_sop_report call.
    """

    id: str = Field(description="SOP record ID (format: sensor_id::start_time::end_time)")
    includes: list[str] | None = Field(default=None, description="Metadata fields to include (ignored for SOP)")
...
async def video_analytics(_config: VideoAnalyticsToolConfig, _builder: Builder) -> AsyncGenerator[FunctionGroup]:
    ...
    async def _get_sensor_ids(input: GetSensorIdsInput) -> list[str]:
        ...
        place = input.place
        if place in ("None", "", "null", "undefined"):
            place = None
    ...
    def _iso_from_offset(msg: dict[str, Any], offset_key: str) -> str | None:
        """Build an ISO-8601 string from first_timestamp (Unix seconds) +
        offset_key (chunk-relative seconds). Returns None if either is missing."""
        first_ts = msg.get("first_timestamp")
        offset = msg.get(offset_key)
        if first_ts is None or offset is None:
            return None
        try:
            dt = datetime.fromtimestamp(float(first_ts) + float(offset), tz=UTC)
            return dt.strftime("%Y-%m-%dT%H:%M:%S.") + f"{dt.microsecond // 1000:03d}Z"
        except (TypeError, ValueError):
            return None

    def _message_timestamp(msg: dict[str, Any]) -> Any:
        # Prefer ISO fields from ES; fall back to deriving from chunk offsets so
        # the LLM never sees a bare float that it might pass to template_report_gen.
        return msg.get("@timestamp") or msg.get("timestamp") or _iso_from_offset(msg, "start_time")

    def _message_end(msg: dict[str, Any]) -> Any:
        # NOTE: "end_time" in mdx-vlm-captions is a chunk-relative float, NOT an
        # ISO date. Only "end" is the ISO field (set by Logstash for non-SOP types).
        # CRITICAL: never return the bare chunk-relative float end_time — the report
        # agent maps this onto template_report_gen's alert_to_timestamp.
        iso_end = msg.get("end")
        if iso_end:
            return iso_end
        # first_timestamp + end_time only yields an ABSOLUTE time for FILE inputs
        # (first_timestamp = wall-clock Unix seconds). For LIVE inputs (rtsp/camera)
        # first_timestamp is a pipeline-relative value (~0), so the sum mis-casts to
        # 1970 — which becomes alert_to_timestamp and produces an invalid clip window
        # (VST replay HTTP 404 → "error generating"). Trust the offset-derived value
        # only when it lands in a sane (post-2000) range.
        cand = _iso_from_offset(msg, "end_time")
        if cand and cand >= "2000":
            return cand
        # Otherwise anchor on the absolute start (@timestamp) and add the chunk
        # duration (end_time - start_time) so the report agent gets a valid, forward
        # [from, to] window inside recorded footage.
        start_iso = msg.get("@timestamp") or msg.get("timestamp")
        if start_iso:
            try:
                from datetime import timedelta
                start_dt = datetime.fromisoformat(str(start_iso).replace("Z", "+00:00"))
                duration = 2.0
                st, en = msg.get("start_time"), msg.get("end_time")
                if st is not None and en is not None:
                    delta = float(en) - float(st)
                    if delta > 0:
                        duration = delta
                end_dt = start_dt + timedelta(seconds=duration)
                return end_dt.strftime("%Y-%m-%dT%H:%M:%S.") + f"{end_dt.microsecond // 1000:03d}Z"
            except (TypeError, ValueError):
                pass
        return start_iso

    def _message_sensor_id(msg: dict[str, Any]) -> str | None:
    ...
    def _message_sensor_type(msg: dict[str, Any]) -> str | None:
    ...
    def _message_response(msg: dict[str, Any]) -> str:
        response = msg.get("response")
        if isinstance(response, str):
            return response
        llm_queries = msg.get("llm", {}).get("queries", [])
        if llm_queries:
            return llm_queries[0].get("response", "")
        return ""

    def _message_checker(msg: dict[str, Any]) -> dict[str, Any]:
        checker_raw = msg.get("checker_result")
        if checker_raw is None:
            info = msg.get("info")
            if isinstance(info, dict):
                checker_raw = info.get("checker_result")

        if isinstance(checker_raw, dict):
            return checker_raw
        if isinstance(checker_raw, str) and checker_raw:
            try:
                checker = json.loads(checker_raw)
            except json.JSONDecodeError:
                return {"raw": checker_raw}
            return checker if isinstance(checker, dict) else {"raw": checker}
        return {}

    def _message_flat_or_info(msg: dict[str, Any], key: str) -> Any:
        if key in msg:
            return msg.get(key)
        info = msg.get("info")
        if isinstance(info, dict):
    ...
    async def _get_sop_status(input: GetSOPStatusInput) -> dict:
        """
        Get the latest SOP (Standard Operating Procedure) VisionLLM status messages from the system.
        Returns the most recent VisionLLM analysis results including current detected action,
        checker results, cycle info, and any SOP violations.

        Args:
            input: Optional sensor_id, start_time, end_time, max_count filters

        Returns:
            dict: List of recent SOP status messages with sensor info, actions, and checker results
        """
        query = deepcopy(BASE_QUERY_TEMPLATE)

        if input.sensor_id:
            query["query"]["bool"]["must"].append(
                {"wildcard": {"sensor_id.keyword": f"*{input.sensor_id}*"}}
            )

        if input.start_time:
            query["query"]["bool"]["must"].append(
                {"range": {"@timestamp": {"gte": input.start_time}}}
            )
        if input.end_time:
            query["query"]["bool"]["must"].append(
                {"range": {"@timestamp": {"lte": input.end_time}}}
        ...
        fetch_size = input.max_count + 1
        messages = await es_client.search(
            index_key="vision_llm_messages",
            query_body=query,
            size=fetch_size,
            sort="@timestamp:desc",
        )
        ...
        results = []
        for msg in messages[:input.max_count]:
            checker = _message_checker(msg)
            results.append({
                "timestamp": _message_timestamp(msg),
                "end": _message_end(msg),
                "sensor_id": _message_sensor_id(msg),
                "sensor_type": _message_sensor_type(msg),
                "current_action": _message_response(msg),
                "cycle": checker.get("cycle"),
                "cycle_completed": checker.get("cycle_completed"),
                "missing_detected": checker.get("missing_detected", []),
                "misordered_detected": checker.get("misordered_detected", []),
                "final_missing_detected": checker.get("final_missing_detected", []),
                "final_misordered_detected": checker.get("final_misordered_detected", []),
                "checker_id": checker.get("checker_id"),
                "frame_number": _message_flat_or_info(msg, "frame_number"),
                "chunk_idx": _message_flat_or_info(msg, "chunk_idx"),
                "cv_execute_time": msg.get("cv_execute_time"),
                "vlm_execute_time": msg.get("vlm_execute_time"),
                # NOTE: start_offset_s / end_offset_s are chunk-relative seconds
                # (floats), NOT absolute timestamps. Renamed from start_time/
                # end_time so the LLM does not pass them to alert_*_timestamp.
                "start_offset_s": msg.get("start_time"),
                "end_offset_s": msg.get("end_time"),
            })

        has_more = len(messages) > input.max_count
        return {"messages": results, "has_more": has_more, "count": len(results)}

    async def _get_sop_report(input: GetSOPReportInput) -> dict:
        """
        Generate a comprehensive SOP compliance report by aggregating all VisionLLM messages
        for a sensor within a time range. The report includes: total messages analyzed,
        cycle progression, all detected SOP violations (missing steps, misordered steps),
        unique actions observed, timing statistics, and a formatted markdown report.

        Use this tool when the user asks to 'Generate an SOP compliance report for sensor
        <sensor_id> from <start_time> to <end_time>'.

        Args:
            input: Optional sensor_id, start_time, end_time filters

        Returns:
            dict: Comprehensive SOP compliance report with structured data and formatted_report markdown
        """
        query = deepcopy(BASE_QUERY_TEMPLATE)
        ...
        if not messages:
            return {"error": "No VisionLLM messages found for the given filters."}
        ...
        all_misordered = []
        all_final_missing = []
        all_final_misordered = []
        first_ts = None
        last_ts = None
        cycle_completions = []

        for msg in messages:
            sid = _message_sensor_id(msg)
            if sid:
                sensor_ids.add(sid)
            ...
            action = _message_response(msg)
            if action:
                actions.append({"timestamp": ts, "action": action})

            checker = _message_checker(msg)
            if checker:
                cycle = checker.get("cycle", 0)
                if cycle > max_cycle:
                    max_cycle = cycle

                if checker.get("cycle_completed"):
                    completed_cycles += 1
                    cycle_completions.append({"cycle": cycle, "timestamp": ts})

                missing = checker.get("missing_detected", [])
                if missing:
                    all_missing.append({"timestamp": ts, "cycle": cycle, "steps": missing})

                misordered = checker.get("misordered_detected", [])
                if misordered:
                    all_misordered.append({"timestamp": ts, "cycle": cycle, "steps": misordered})

                final_missing = checker.get("final_missing_detected", [])
                if final_missing:
                    all_final_missing.append({"timestamp": ts, "cycle": cycle, "steps": final_missing})

                final_misordered = checker.get("final_misordered_detected", [])
                if final_misordered:
                    all_final_misordered.append({"timestamp": ts, "cycle": cycle, "steps": final_misordered})

        unique_actions = list(dict.fromkeys(a["action"] for a in actions))

        total_violations = (
            len(all_missing) + len(all_misordered)
            + len(all_final_missing) + len(all_final_misordered)
        )
        ...
        sensor_display = ", ".join(sorted(sensor_ids)) if sensor_ids else (input.sensor_id or "All Sensors")

        missing_table_rows = []
        for m in all_missing:
            missing_table_rows.append(f"| {m['cycle']} | {m['timestamp']} | {', '.join(str(s) for s in m['steps'])} |")
        for m in all_final_missing:
            missing_table_rows.append(f"| {m['cycle']} (final) | {m['timestamp']} | {', '.join(str(s) for s in m['steps'])} |")
        missing_table = "\n".join(missing_table_rows) if missing_table_rows else "| - | - | No missing steps detected |"

        misordered_table_rows = []
        for m in all_misordered:
            misordered_table_rows.append(f"| {m['cycle']} | {m['timestamp']} | {', '.join(str(s) for s in m['steps'])} |")
        for m in all_final_misordered:
            misordered_table_rows.append(f"| {m['cycle']} (final) | {m['timestamp']} | {', '.join(str(s) for s in m['steps'])} |")
        misordered_table = "\n".join(misordered_table_rows) if misordered_table_rows else "| - | - | No misordered steps detected |"

        latest_action_str = actions[-1]["action"] if actions else "N/A"
        unique_actions_str = ", ".join(unique_actions) if unique_actions else "N/A"

        formatted_report = f"""# SOP Compliance Report

## Basic Information

| Field | Value |
|-------|-------|
| **Sensor ID** | {sensor_display} |
| **Time Range** | {first_ts or 'N/A'} to {last_ts or 'N/A'} |
...
| **Current Cycle** | {max_cycle} |
| **Completed Cycles** | {completed_cycles} |

## SOP Violations

### Missing Steps ({len(all_missing) + len(all_final_missing)} occurrences)

| Cycle | Timestamp | Missing Steps |
|-------|-----------|---------------|
{missing_table}

### Misordered Steps ({len(all_misordered) + len(all_final_misordered)} occurrences)
...
| **Unique Actions** | {unique_actions_str} |
| **Latest Action** | {latest_action_str} |
...
**End of Report**"""
        ...
        return {
            ...
            "sop_violations": {
                "missing_detected_count": len(all_missing),
                "missing_detected": all_missing,
                "misordered_detected_count": len(all_misordered),
                "misordered_detected": all_misordered,
                "final_missing_detected_count": len(all_final_missing),
                "final_missing_detected": all_final_missing,
                "final_misordered_detected_count": len(all_final_misordered),
                "final_misordered_detected": all_final_misordered,
            },
            "actions_observed": {
                "total_action_entries": len(actions),
    ...
    def _clean_sensor_id(raw_sensor_id: str | None) -> str | None:
        """Extract short sensor name from a full RTSP URL or return as-is."""
        if not raw_sensor_id:
            return raw_sensor_id
        if raw_sensor_id.startswith("rtsp://"):
            return raw_sensor_id.rstrip("/").rsplit("/", 1)[-1]
        return raw_sensor_id

    async def _get_sop_as_incidents(input: GetSOPAsIncidentsInput) -> dict:
        """
        Adapter that returns a single window-level SOP compliance "incident" for
        the report_agent (which expects incident-style responses).

        The report_agent's single-incident mode calls this tool with max_count=1
        and hands incidents[0] straight to template_report_gen as the report
        metadata. A per-message mapping made every report describe a single
        VisionLLM caption (Total Actions Recorded == 1, Total Messages Analyzed
        == Unknown). Instead, aggregate the entire [start_time, end_time] window
        via get_sop_report and expose those totals so the generated report
        reflects every message, action, and violation in range.

        Args:
            input: Incident-style parameters (source, source_type, start_time, end_time, max_count)

        Returns:
            dict: Incidents-compatible response carrying one aggregate SOP incident
        """
        sensor_id = input.source if input.source_type == "sensor" else input.source

        report = await _get_sop_report(
            GetSOPReportInput(
                sensor_id=sensor_id,
                start_time=input.start_time,
                end_time=input.end_time,
        ...
        clean_sid = _clean_sensor_id(sensor_id) or sensor_id

        # No data in range: return a single empty placeholder incident so the
        # report_agent can still produce a (compliant/empty) report instead of
        # erroring out.
        if "error" in report:
            composite_id = f"{clean_sid or 'unknown'}::{input.start_time or ''}::{input.end_time or ''}"
            return {
                "incidents": [
                    {
                        "Id": composite_id,
                        "id": composite_id,
                        "timestamp": input.start_time,
                        "end": input.end_time,
                        "sensorId": clean_sid,
                        "objectIds": [],
                        "info": {"type": "sop_compliance", **report},
                    }
                ],
                "has_more": False,
            }
        ...
        time_range = report_summary.get("time_range", {})
        actions_observed = report.get("actions_observed", {})

        # Anchor the report sensor on what the aggregation actually observed.
        report_sensors = report_summary.get("sensors") or []
        resolved_sid = _clean_sensor_id(report_sensors[0]) if report_sensors else clean_sid

        # Use the actual observed range for the clip/snapshot window, falling
        # back to the requested window when the aggregation did not surface it.
        start_iso = time_range.get("start") or input.start_time
        end_iso = time_range.get("end") or input.end_time
        composite_id = f"{resolved_sid or 'unknown'}::{input.start_time or ''}::{input.end_time or ''}"

        latest_action = actions_observed.get("latest_action") or {}

        # Embed the full aggregated report so template_report_gen can populate
        # Total Messages Analyzed, Total Actions Recorded, Unique Actions,
        # violations, cycle status, etc. for the whole window. The flat fields
        # and formatted_report give the LLM the exact values to copy.
        incident = {
            "Id": composite_id,
            "id": composite_id,
            "timestamp": start_iso,
            "end": end_iso,
            "sensorId": resolved_sid,
            "objectIds": [],
            "info": {
                "type": "sop_compliance",
                "total_messages_analyzed": report_summary.get("total_messages_analyzed"),
                "total_actions_recorded": actions_observed.get("total_action_entries"),
                "unique_actions": actions_observed.get("unique_actions", []),
                "latest_action": latest_action.get("action"),
                "current_cycle": report_summary.get("current_cycle"),
                "completed_cycles": report_summary.get("completed_cycles"),
                "compliance_status": report_summary.get("compliance_status"),
                "time_range": time_range,
                "sop_violations": report.get("sop_violations", {}),
                "report_summary": report_summary,
                "actions_observed": actions_observed,
                "formatted_report": report.get("formatted_report"),
            },
        }

        return {"incidents": [incident], "has_more": False}

    async def _get_sop_as_incident(input: GetSOPAsIncidentInput) -> dict:
        """
        Adapter that wraps get_sop_report to return data in incident-like format.
        Used by report_agent which expects single-incident-style responses.

        Parses the composite ID (sensor_id::start_time::end_time) and calls
        get_sop_report with the extracted parameters.

        Args:
            input: Incident-style parameters (id with encoded sensor/time info)

        Returns:
            dict: Single incident-compatible response with comprehensive SOP data
        """
        parts = input.id.split("::")
        sensor_id = parts[0] if len(parts) > 0 and parts[0] not in ("unknown", "all", "") else None
        start_time = parts[1] if len(parts) > 1 and parts[1] else None
        end_time = parts[2] if len(parts) > 2 and parts[2] else None

        sop_input = GetSOPReportInput(
            sensor_id=sensor_id,
            start_time=start_time,
            end_time=end_time,
        )
        result = await _get_sop_report(sop_input)
        ...
        return {
            "Id": input.id,
            "id": input.id,
            "timestamp": time_range.get("start", start_time),
            "end": time_range.get("end", end_time),
            "sensorId": sensor_id,
            "objectIds": [],
            "info": result,
        }
    ...
    if "get_sop_status" in _config.include:
        group.add_function(name="get_sop_status", fn=_get_sop_status, description=_get_sop_status.__doc__)

    if "get_sop_report" in _config.include:
        group.add_function(name="get_sop_report", fn=_get_sop_report, description=_get_sop_report.__doc__)

    if "get_sop_as_incidents" in _config.include:
        group.add_function(name="get_sop_as_incidents", fn=_get_sop_as_incidents, description=_get_sop_as_incidents.__doc__)

    if "get_sop_as_incident" in _config.include:
        group.add_function(name="get_sop_as_incident", fn=_get_sop_as_incident, description=_get_sop_as_incident.__doc__)
