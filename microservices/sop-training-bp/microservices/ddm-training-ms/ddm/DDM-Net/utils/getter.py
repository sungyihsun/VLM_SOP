...
class data_prefetcher:
    ...
    def preload(self):
        try:
            ...
            if "current_ids" in self.nextitem:
                self.next_current_ids = self.nextitem["current_ids"]
            else:
                self.next_current_ids = None
    ...
    def next(self):
        ...
        path = self.next_path
        current_ids = self.next_current_ids
        self.preload()
        return input, target, path, current_ids
...
def getDataLoader(dataset, is_training=True, args=None):
    ...
    # Use custom collate_fn if provided, otherwise use default my_collate_fn
    collate_fn = getattr(args, 'collate_fn', my_collate_fn)
