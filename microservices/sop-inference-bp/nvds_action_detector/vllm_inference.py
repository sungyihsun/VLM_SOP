...
class VLLMInference:
    ...
    def _preprocess_video(
        ...
        **kwargs,
        ...
        original_video_fps = float(kwargs.get("video_fps") or 0.0)
        if original_video_fps <= 0:
            fallback_fps = float(os.environ.get("RTSP_FALLBACK_FPS") or os.environ.get("CAMERA_FPS_NUM") or VLM_FPS or 30.0)
            logger.warning(
                f"Invalid source video_fps={original_video_fps}; using fallback fps={fallback_fps} for VLM preprocessing"
            )
            original_video_fps = fallback_fps
        ...
        if self._is_qwen3vl:
            ...
            video_metadata = dict(
                ...
                duration=total_frames / original_video_fps if original_video_fps else 0.0,
